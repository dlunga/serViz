...
//Enabling the Return/Enter button interactivity.
...
function computeService() {	
 ...
 //2013-05-03 14:53:00
  ...
  function serviceTable(div) {
    ...
    //denestData(commentsNestByBranches);
      ...
      div.each(function() {
       ...
       var tr = d3.select(this)
		...
		var rects =	 tc_svg.selectAll("td")
					    ...
					    //.attr("style", function(d, i) { return d.comment_sentiment == "positive" ? "stroke:#2ca02c; fill:#2ca02c" : "stroke:#d62728;fill:#d62728"; }) // color code rectangles
					    .append("svg:title")
...
function denestData(data){
	...
	/*
	for (key in data) {
    	//if (typeof(data[key]) == "object" && data[key] != null&& key != "comment_wordFreq") {
    	if (typeof(data[key]) == "object" && data[key] != null) {
      		denestData(data[key]);
    	} 
    	else{
    	//else if (key == 'comment_wordFreq'){
    		//console.log("Key: " + key);  
...
function sentimentByBranches(){
  ...
  //console.log(gp_branches_dict_neg.sortedKeys);
			  ...
			  function selectHandler() {
		          var selectedItem = chart_sentiment.getSelection()[0];
		          	if (selectedItem) {
		            		...
		            		alert('The user selected ' + provinceprovinceSelected );
		          	}
		      }
...
function drawBranchSentimentChart() {
	...
	*/
	...
	for (var i = 0; i < keyneg.length; i++) {
		dataneg.addRow([keyneg[i], gp_branches_dict_neg.dictData[keyneg[i]]]);
	}
	...
	chart_sentiment_neg = new google.visualization.BarChart(document.getElementById('sentiment_div2'));
...
function cloudPlot(){
	...
	canvas.height = surr_div.offsetHeight;
	canvas.width = surr_div.offsetWidth;
	...
	//WordCloud(document.getElementById('canvas_cloud'), { list: wordfreq } );
}
