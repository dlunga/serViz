...
for (var i = 0; i < serviceComments.length; i++) {
  comment = serviceComments[i];
  latLng = new google.maps.LatLng(comment.latitude, comment.longitude);
...
function computeService() {	
  ...
  charts = [   
   barChart()
     ...
     barChart()
        ...
        .x(d3.time.scale()
        .domain([new Date(2013, 4, 1), new Date(2014, 12, 1)])
        .rangeRound([0, 10 * 90]))
 ...
 // Render the initial service list.
  servicelist = d3.select("#reports").data([serviceList]);//.data([serviceColors]).; 
    ...
    var rects;
      div.each(function() {
       ...
       rects =	svg.selectAll("rect")
		...
		//data = rects.select(function(d) { return {lat: d.latitude, lon:d.longitude}; });							    			    			
       logOrdered.exit().remove();
       logOrdered.order();      	
    });
    ...
    var rects;
      div.each(function() {
       ...
       var tr = d3.select(this)
  				.selectAll("tr") //FIX THIS "div" or ".date"
  				.data(serviceBySuppliers,function(d) { return d.key; });
    	tr.enter().append("tr")
    	            .append("th")
    				.attr("class", "name")
		...
		rects =	 tc_svg.selectAll("td")
					    ...
					    .html(function(d) { return d.consumer_comment; });
      /*					
 	 	var tc2 = tc.selectAll("td")
            		.data(function(d2) { return d2.values; })
            		.enter().append("td")
            		.text(function(d4) { return +d4.values;});
