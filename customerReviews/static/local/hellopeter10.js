...
function computeService() {	
 ...
 function handleClick(){
	...
	if (myArray.length > 1){	
		serviceBySupplier.filterRange(myArray).top(Infinity);
		}
		 
	else if (myArray[0].length> 1){
		myArray = document.getElementById("supplierInput").value.toUpperCase();
		serviceBySupplier.filter(myArray).top(Infinity);
	}
      ...
      div.each(function() {
       ...
       var tr = d3.select(this)
		...
		rects =	 tc_svg.selectAll("td")
					    ...
					    .attr("rx","15")
					    .attr("ry", "2")
					    .attr("style", function(d, i) { return d.comment_score == "1" ? "stroke:#2ca02c; fill:#2ca02c" : "stroke:#d62728;fill:#d62728"; })
