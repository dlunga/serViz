...
function computeservice(services,flights) {	
  // Various formatters.
  ...
  // my comments
  // D:edits
  var service = crossfilter(services),
      service_all = service.groupAll(),
      service_date = service.dimension(function(d) { return d.comment_datetime; }),
      service_dates = service_date.group(d3.time.day),
      service_hour = service.dimension(function(d) { return d.comment_datetime.getHours() + d.comment_datetime.getMinutes() / 60; }),
      service_hours = service_hour.group(Math.floor);
      //delay = comment.dimension(function(d) { return Math.max(-60, Math.min(149, d.delay)); }),
      //delays = delay.group(function(d) { return Math.floor(d / 10) * 10; }),
      //distance = comment.dimension(function(d) { return Math.min(1999, d.distance); }),
      //distances = distance.group(function(d) { return Math.floor(d / 50) * 50; });  
  ...
  var charts = [
   ...
   barChart()
     ...
     barChart()
        ...
        .round(d3.time.day.round)
      .x(d3.time.scale()
        .domain([new Date(2013, 2, 1), new Date(2014, 9, 1)])
        .rangeRound([0, 10 * 90]))
  ...
  //var list = d3.select("#flight-list")
  //    .data([flightList]);
      
  // Render the initial service list.
  var servicelist = d3.select("#service-list")
      .data([serviceList]);
  ...
  function serviceList(div) {
    ...
    /*
     serviceEnter.append("div")
      ...
      serviceEnter.append("div")
          .attr("class", "latitude")
          .text(function(d) { return formatChange(d.latitude); }); */
